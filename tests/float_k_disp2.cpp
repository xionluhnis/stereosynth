...
int main() {
    ...
    search.minimum = 7.0f;
    auto seq = VerboseAlgorithm() << HorizontalSearch<Patch2tf, float, KNNF_K>(&nnf)
                           ...
                           << LocalMean<Patch2tf, float, KNNF_K, 4>(&nnf, 1)
                           << LocalMean<Patch2tf, float, KNNF_K, 4>(&nnf, 2)
                           << LocalMean<Patch2tf, float, KNNF_K, 8>(&nnf, 1)
                           << LocalMean<Patch2tf, float, KNNF_K, 8>(&nnf, 2)
                           << LocalMean<Patch2tf, float, KNNF_K, 16>(&nnf, 1);
    NoOp<Point2i, bool, false> filter;
    ConvergenceDiary::Data convData;
    auto post = PostSequence()  << DecreasingSearchRadius<float>(&search)
                                << ConvergenceDiary(&seq, &convData);
